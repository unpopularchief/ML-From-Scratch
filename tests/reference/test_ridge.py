...
def test_matches_sklearn_on_a_noisy_problem() -> None:
    ...
    ours = Ridge(alpha=1.0).fit(X, y)
    theirs = linear_model.Ridge(alpha=1.0).fit(X, y)
...
def test_matches_sklearn_across_alphas() -> None:
    ...
    X, y = make_regression(n_samples=150, n_features=4, noise=0.2, random_state=2)

    for alpha in (0.1, 1.0, 10.0, 100.0):
        ours = Ridge(alpha=alpha).fit(X, y)
        theirs = linear_model.Ridge(alpha=alpha).fit(X, y)
        np.testing.assert_allclose(ours.coef_, theirs.coef_, rtol=1e-6)
        np.testing.assert_allclose(ours.intercept_, theirs.intercept_, rtol=1e-6)
...
def test_matches_sklearn_without_intercept() -> None:
    ...
    ours = Ridge(alpha=1.0, fit_intercept=False).fit(X, y)
    theirs = linear_model.Ridge(alpha=1.0, fit_intercept=False).fit(X, y)
